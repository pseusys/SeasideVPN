...
from .utils import ProtocolMessageType, ProtocolParseError, ProtocolReturnCode, ProtocolTerminationError, monitor_task
...
class PortClient(_PortPeer, SeasideClient):
    ...
    async def connect(self, callback: Optional[ReceiveCallback] = None):
        ...
        if callback is not None:
            ...
            self._data_callback = monitor_task(self._read_cycle(callback), name=self._CALLBACK_TASK_NAME)
...
class PortServer(_PortPeer, SeasidePeer):
    ...
    def __init__(self, key: bytes, connection: socket, local: Optional[IPv4Address] = None, timeout: Optional[float] = None):
...
class PortListener(SeasideListener):
    ...
    async def listen(self, connection_callback: Optional[ConnectionCallback] = ProtocolReturnCode, data_callback: Optional[ServeCallback] = None):
        ...
        while True:
            ...
            server = PortServer(key, connection, self._listener_address, self._timeout)
