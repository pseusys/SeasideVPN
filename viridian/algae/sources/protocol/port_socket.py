...
class _PortPeer:
    ...
    async def read(self) -> bytes:
        ...
        if type == MessageType.DATA:
            ...
            value = self._core.parse_any_any_data(self._symmetric, data)
...
class PortListener(SeasideListener):
    ...
    async def listen(self, connection_callback: Optional[ConnectionCallback] = TyphoonReturnCode, data_callback: Optional[ServeCallback] = None):
        ...
        packet = self._core.build_server_init(cipher, user_id, status)
        ...
        server = PortServer(key, connection, self._core._default_timeout)
