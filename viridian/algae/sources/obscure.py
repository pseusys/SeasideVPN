...
def obfuscate(gravity: int, encrypted_packet: Union[bytes, UserControlRequestStatus], user_token: Optional[int] = None) -> bytes:
    ...
    tail = get_random_bytes(tail_length)
...
def deobfuscate(gravity: int, obfuscated_packet: bytes) -> Tuple[bytes, Optional[int]]:
    signature = obfuscated_packet[0] ^ gravity
    payload_end = len(obfuscated_packet) - int(signature // 2)
    if signature % 2 == 1:
