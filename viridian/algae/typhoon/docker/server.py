...
parser.add_argument("CERTIFICATE", type=str, help=f"Server encryption certificate (private key + public key), encoded with base64")
parser.add_argument("-a", "--address", default="0.0.0.0", type=IPv4Address, help=f"Server encryption certificate (private key + public key), encoded with base64")
parser.add_argument("-p", "--port", default="0", type=int, help=f"Server encryption certificate (private key + public key), encoded with base64")
...
async def connection_callback(user_type: str, user_server: TyphoonServer, user_token: bytes) -> TyphoonReturnCode:
    logger.info(f"Connected user '{user_type}', index {user_server.user_id} with token: {user_token}")
    await sleep(0.1)
...
async def data_callback(user_idx: int, data: bytes) -> None:
    logger.info(f"Message received from user {user_idx}, data: {data}")
    await servers[user_idx].write(data)


async def main(args: List[str] = argv[1:]):
