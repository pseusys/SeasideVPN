...
def test_tunnel_init(tunnel: Tunnel) -> None:
    ...
    ip_links = run(["ip", "link", "show"], text=True, capture_output=True, check=True).stdout
...
def test_tunnel_up(tunnel: Tunnel) -> None:
    ...
    ip_addresses = run(["ip", "addr", "show"], text=True, capture_output=True, check=True).stdout
