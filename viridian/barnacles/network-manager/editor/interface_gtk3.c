...
static void choose_certificate_cb(GtkWidget* button __attribute__((unused)), gpointer user_data) {
	...
	GtkWidget* dialog = gtk_file_chooser_dialog_new("Select Seaside Certificate", NULL, GTK_FILE_CHOOSER_ACTION_OPEN, "_Cancel", GTK_RESPONSE_CANCEL, "_Open", GTK_RESPONSE_ACCEPT, NULL);
...
static gboolean init_editor_plugin(SeasideEditor* self, NMConnection* connection, GError** error) {
	...
	if (s_vpn) {
		...
		if (proto_value) {
			if (g_strcmp0(proto_value, "typhoon") == 0) gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(priv->radio_typhoon), TRUE);
			else if (g_strcmp0(proto_value, "port") == 0) gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(priv->radio_port), TRUE);
			else {
