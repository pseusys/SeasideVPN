...
static gboolean capture_error_idle(gpointer data) {
	...
	} else g_debug("DBUS runtime: SeasideVPN interface exited cleanly!");
...
static void capture_error(void* plugin_ptr, char* error) {
...
static gboolean seaside_load_library(NMSeasidePluginPrivate* priv, GError** error) {
	if (priv->lib_handle) return TRUE;

	priv->lib_handle = dlopen(LIB_BASENAME, RTLD_NOW | RTLD_LOCAL);
	if (!priv->lib_handle) {
		g_set_error(error, NM_VPN_PLUGIN_ERROR, NM_VPN_PLUGIN_ERROR_LAUNCH_FAILED, "Error loading libseaside: %s", dlerror());
		return FALSE;
	}
...
static gboolean seaside_set_vpnconfig_idle(gpointer user_data) {
...
static gboolean real_connect(NMVpnServicePlugin* plugin, NMConnection* connection, GError** error) {
	...
	if (!s_vpn) {
		...
		g_set_error(error, NM_VPN_PLUGIN_ERROR, NM_VPN_PLUGIN_ERROR_INVALID_CONNECTION, "Error extracting settings");
		return FALSE;
	}
...
static gboolean empty_need_secrets(NMVpnServicePlugin* plugin __attribute__((unused)), NMConnection* connection __attribute__((unused)), const char** setting_name __attribute__((unused)), GError** error __attribute__((unused))) {
	g_debug("DBUS need secrets: Skipped!");
	return FALSE;
}

static gboolean empty_new_secrets(NMVpnServicePlugin* base_plugin __attribute__((unused)), NMConnection* connection __attribute__((unused)), GError** error __attribute__((unused))) {
	g_debug("DBUS new secrets: Skipped!");
	return TRUE;
}

static void nm_seaside_plugin_init(NMSeasidePlugin* plugin) {
...
static void nm_seaside_plugin_class_init(NMSeasidePluginClass* klass) {
...
NMSeasidePlugin* nm_seaside_plugin_new(void) {
	GError* error = NULL;
	NMSeasidePlugin* plugin = g_initable_new(NM_TYPE_SEASIDE_PLUGIN, NULL, &error, NM_VPN_SERVICE_PLUGIN_DBUS_SERVICE_NAME, NM_DBUS_SERVICE_SEASIDE, NULL);
	if (!plugin) {
