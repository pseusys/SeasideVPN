...
pub fn parse_client_link(link: String) -> DynResult<(String, Vec<u8>, Option<u16>, Option<u16>, Vec<u8>)> {
    ...
    let parsed_public = URL_SAFE_NO_PAD.decode(&raw_public)?;

    let raw_token = require_with!(query.get("token"), "Viridian token query not found!").as_bytes();
    let parsed_token = URL_SAFE_NO_PAD.decode(&raw_token)?;
