...
pub async fn build_any_data<'a>(cipher: &mut Symmetric, data: ByteBuffer<'a>) -> DynResult<ByteBuffer<'a>> {
    ...
    log::error!("Packet header slice created: {:?}", header_buffer.slice());  // TODO: REMOVE!!!
    let encrypted_header = cipher.encrypt(header_buffer, None)?;
    log::error!("Packet header slice encrypted: {:?} bytes", encrypted_header.len());  // TODO: REMOVE!!!
    let encrypted_header_with_body = encrypted_header.expand_end(encrypted_data_len);
    log::error!("Packet data slice ready: {:?} bytes", encrypted_header_with_body.len());  // TODO: REMOVE!!!
