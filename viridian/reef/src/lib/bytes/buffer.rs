...
impl<'a> ByteBuffer<'a> {
    ...
    pub fn empty(size: usize) -> Self {
        ByteBuffer { data: Rc::new(RefCell::new(KeptVector::new(size))), length: size, start: 0, end: size }
    }
...
impl<'a> ByteBuffer<'a> {
    pub fn rebuffer_start(&self, start: usize) -> Self {
        ...
        ByteBuffer { data: self.data.clone(), length: self.length, start: new_start, end: self.end }
    }
    ...
    pub fn rebuffer_end(&self, end: usize) -> Self {
        ...
        ByteBuffer { data: self.data.clone(), length: self.length, start: self.start, end: new_end }
    }
    ...
    pub fn rebuffer_both(&self, start: usize, end: usize) -> Self {
        ...
        ByteBuffer { data: self.data.clone(), length: self.length, start: new_start, end: new_end }
    }
    ...
    pub fn split_buf(&self, divide: usize) -> (Self, Self) {
        ...
        (ByteBuffer { data: self.data.clone(), length: self.length, start: self.start, end: new_divide }, ByteBuffer { data: self.data.clone(), length: self.length, start: new_divide, end: self.end })
    }
...
impl<'a> From<Vec<u8>> for ByteBuffer<'a> {
    fn from(value: Vec<u8>) -> Self {
        ...
        ByteBuffer { data: Rc::new(RefCell::new(KeptVector::from(value))), length: length, start: 0, end: length }
    }
}
...
impl<'a> From<&[u8]> for ByteBuffer<'a> {
    fn from(value: &[u8]) -> Self {
        ...
        ByteBuffer { data: Rc::new(RefCell::new(KeptVector::from(vector))), length: length, start: 0, end: length }
    }
}
...
impl<'a> Clone for ByteBuffer<'a> {
    #[inline]
    fn clone(&self) -> Self {
        ByteBuffer { data: self.data.clone(), length: self.length, start: self.start, end: self.end }
    }
}
