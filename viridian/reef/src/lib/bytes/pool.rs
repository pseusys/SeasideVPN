...
impl KeptVector<'_> {
    pub fn new(size: usize) -> Self {
        KeptVector { data: vec![0u8; size], pool: None }
    }
}

impl<'a> From<Vec<u8>> for KeptVector<'a> {
    fn from(value: Vec<u8>) -> Self {
        KeptVector { data: value, pool: None }
    }
}
...
impl<'a> BytePool {
    pub fn new(before_cap: usize, size: usize, after_cap: usize, initial: usize) -> Self {
        ...
        BytePool { before_cap, size, after_cap, capacity, pool: Mutex::new(vec![vec![0u8; capacity]; initial]) }
    }
